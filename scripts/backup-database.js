...
if (!connectionString) {
  ...
  console.error('   Use: DATABASE_URL="postgresql://..." node scripts/backup-database.js');
...
async function backupDatabase() {
  ...
  try {
    ...
    backupContent += `-- Database: ${connectionString.split('@')[1]?.split('/')[1] || 'neo_smart_factory'}\n`;
    ...
    for (const { tablename } of tables) {
      ...
      if (rows.length > 0) {
        ...
        for (const row of rows) {
          ...
          backupContent += `INSERT INTO ${tablename} (${columnNames.join(', ')}) VALUES (${values});\n`;
        }
