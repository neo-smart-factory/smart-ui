...
export function useSchemaValidation() {
    ...
    const validate = (data) => {
        ...
        if (data.tokenName && data.tokenName.length < (inputSchema.properties.name?.minLength || 3)) {
            return `Protocol Identity must be at least ${inputSchema.properties.name?.minLength || 3} characters.`;
        }

        if (data.tokenSymbol && (data.tokenSymbol.length < 2 || data.tokenSymbol.length > 6)) {
