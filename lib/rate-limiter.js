...
const MAX_MAP_SIZE = 10_000; // Limite para prevenir OOM
...
export function rateLimiter(req, res, next) {
  ...
  const now = Date.now();
  // Usar apenas IP como chave - incluir req.url permite DoS por variação de query string
  const key = `${ip}`;

  // Limite de tamanho do Map para prevenir exaustão de memória
  if (!requestCounts.has(key) && requestCounts.size >= MAX_MAP_SIZE) {
    const firstKey = requestCounts.keys().next().value;
    requestCounts.delete(firstKey);
  }
