...
export async function searchTavily(query, options = {}) {
  ...
  // SEGURANÇA: API key nunca deve ir no body - usar header de autenticação
  ...
  if (!response.ok) {
    // SEGURANÇA: Não expor detalhes internos no erro
...
export function extractTrends(answer) {
  ...
  return answer
    ...
    .filter((sentence) => {
      ...
      return (
        lower.length > 20 &&
        trendKeywords.some((keyword) => lower.includes(keyword))
      );
    })
    .slice(0, 5)
    .map((s) => s.trim());
...
export function identifyMarketGaps(answer) {
  ...
  return answer
    ...
    .filter((sentence) => {
      ...
      return (
        lower.length > 20 &&
        gapIndicators.some((indicator) => lower.includes(indicator))
      );
    })
    .slice(0, 3)
    .map((s) => s.trim());
