...
-Garanta que a ABI no frontend (`smart-ui`) corresponde ao deploy atual. -**Nota:** Use variável de ambiente `CORE_CONTRACTS_PATH` ou caminho relativo padrão
...
-Opcionalmente, gere um snippet de marketing em `${INTERNAL_OPS_PATH:-../neo-smart-factory/internal-ops}/marketing/` descrevendo o update. -**Nota:** Configure `INTERNAL_OPS_PATH` no `.env` para seu ambiente
...
-`NEO::doc <mensagem>` -> Adicionar ao changelog. -`NEO::sync` -> Verificar integridade entre UI e Core. -`NEO::ops <status>` -> Atualizar status no Internal Ops.

// turbo-all

## 🚀 Execução Sugerida

1.Ler arquivos de configuração global no repositório `neo-smart-factory`. 2. Identificar discrepâncias de versão. 3. Consolidar documentação.
